...
- `_notify_new_insights()` defines push identity as `(session_key, headline)`. Only newly appearing visible cards notify; unchanged, handled, or checkpoint-restored cards do not. `_dispatch_push()` sends through `PushService` off the event loop.
