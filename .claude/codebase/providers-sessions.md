...
- `SessionProvider.pending_interaction()` returns only a currently actionable Pending Interaction: AgentDeck Ownership, Runtime Availability, and `Capability.INTERACT` must agree. Cached runtime requests remain provider-internal during an outage; callers must still require `INTERACT` before rendering or answering.
...
- `src/agentdeck/state.py::visible_sessions()` hides sessions without confirmed local liveness unless a provider sets `show_when_idle`. `session_presentation()` builds one nesting level and omits orphaned native subagents rather than promoting contextless cards.
