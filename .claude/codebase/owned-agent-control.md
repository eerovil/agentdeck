...
- AgentDeck-owned Claude and Codex sessions receive the same file-presentation contract from `providers/instructions.py`: relative HTML links are cwd-scoped sandbox previews, while links intended to open or download files use absolute filesystem paths.
- Providers expose a Pending Interaction only while the owning runtime is available and the normalized Session has `Capability.INTERACT`. The page, SSE stream, Deckhand, delegation status, and answer route all consume that gated provider interface; cached ownership or a cached runtime request is never an independent control signal.
